...
export function preprocessWikiLinks(markdown, options = {}) {
  if (!markdown) {
    return markdown;
  }
  ...
  for (const match of matches) {
    ...
    if (!!options.wikiLinksPrefix) {
      url = options.wikiLinksPrefix + url;
    }
    if (!!options.wikiLinksSuffix) {
      url = url + options.wikiLinksSuffix;
    }
